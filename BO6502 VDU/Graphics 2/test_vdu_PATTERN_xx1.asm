...
VDP_PATTERN_TABLE_BASE  = $0000      ; PATTERN TABLE OK REG4      00-03
VDP_PATTERN_TABLE_BASE1 = $0800       
VDP_PATTERN_TABLE_BASE2 = $1000
VDP_SPRITE_TABLE_BASE   = $1800      ; SPRITE TABLE  OK REG6      03
VDP_COLOR_TABLE_BASE    = $2000      ; COLOR TABLE   OK REG3      80-FF
VDP_COLOR_TABLE_BASE1   = $2800
VDP_COLOR_TABLE_BASE2   = $3000
VDP_NAME_TABLE_BASE     = $3800      ; NAME TABLE    OK REG2      0E
VDP_SPRITE_ATT_BASE     = $3B00      ; SPRITE ATT    OK REG5      76
...
VDP_SETUP:
  ...
  JSR VDP_INITIALIZE_COLOR_TABLE1
  JSR VDP_INITIALIZE_COLOR_TABLE2
  JSR VDP_INITIALIZE_NAME_TABLE 
  JSR VDP_INITIALIZE_SPRITE_TABLE  
  JSR VDP_INITIALIZE_PATTERN_TABLE
  JSR VDP_INITIALIZE_PATTERN_TABLE1
  JSR VDP_INITIALIZE_PATTERN_TABLE2           
  RTS
...
VDP_INITIALIZE_PATTERN_TABLE1:       
 PHA
 PHX
 LDA #<(VDP_PATTERN_TABLE_BASE1)                      ; Byte 1 loading LSB Pattern Table Address
 STA VREG
 LDA #(VDP_WRITE_VRAM_BIT | >VDP_PATTERN_TABLE_BASE1) ; Byte 2 loading MSB Pattern Table Address (combined
 STA VREG                                            ; with 0 1 A0 A1 A2 A3 A4 A5
 LDA #<VDP_PATTERNS                                  ; LOAD THE START ADDRESS OF THE PATTERNS TO ZERO PAGE
 STA VDP_INIT_A
 LDA #>VDP_PATTERNS
 STA VDP_INIT_HI_A
VDP_PATTERN_TABLE_LOOP1: 
 LDA (VDP_INIT_A)                                    ; LOAD A WITH THE VALUE AT VDP_INIT
 STA VDATA                                           ; AND STORE IT TO VRAM
 LDA VDP_INIT_A                                      ; LOAD THE LOW BYTE OF VDP_INIT ADDRESS INTO A
 CLC                                                 ; CLEAR CARRY FLAG
 ...
 STA VDP_INIT_HI_A                                   ; AND STORE THAT BACK INTO THE HIGH BYTE
 CMP #>VDP_END_PATTERNS                              ; COMPARE IF WE'RE AT THE END OF THE PATTERNS
 BNE VDP_PATTERN_TABLE_LOOP1                          ; IF NOT, LOOP AGAIN
 LDA VDP_INIT_A                                      ; COMPARE THE LOW BYTE
 CMP #<VDP_END_PATTERNS
 BNE VDP_PATTERN_TABLE_LOOP1                          ; IF NOT EQUAL, LOOP AGAIN
 PLX
 PLA
 RTS

VDP_INITIALIZE_PATTERN_TABLE2:       
 PHA
 PHX
 LDA #<(VDP_PATTERN_TABLE_BASE2)                      ; Byte 1 loading LSB Pattern Table Address
 STA VREG
 LDA #(VDP_WRITE_VRAM_BIT | >VDP_PATTERN_TABLE_BASE2) ; Byte 2 loading MSB Pattern Table Address (combined
 STA VREG                                            ; with 0 1 A0 A1 A2 A3 A4 A5
 LDA #<VDP_PATTERNS                                  ; LOAD THE START ADDRESS OF THE PATTERNS TO ZERO PAGE
 STA VDP_INIT_A
 LDA #>VDP_PATTERNS
 STA VDP_INIT_HI_A
VDP_PATTERN_TABLE_LOOP2: 
 LDA (VDP_INIT_A)                                    ; LOAD A WITH THE VALUE AT VDP_INIT
 STA VDATA                                           ; AND STORE IT TO VRAM
 LDA VDP_INIT_A                                      ; LOAD THE LOW BYTE OF VDP_INIT ADDRESS INTO A
 CLC                                                 ; CLEAR CARRY FLAG
 ...
 STA VDP_INIT_HI_A                                   ; AND STORE THAT BACK INTO THE HIGH BYTE
 CMP #>VDP_END_PATTERNS                              ; COMPARE IF WE'RE AT THE END OF THE PATTERNS
 BNE VDP_PATTERN_TABLE_LOOP2                          ; IF NOT, LOOP AGAIN
 LDA VDP_INIT_A                                      ; COMPARE THE LOW BYTE
 CMP #<VDP_END_PATTERNS
 BNE VDP_PATTERN_TABLE_LOOP2                          ; IF NOT EQUAL, LOOP AGAIN
 PLX
 PLA
 RTS
...
VDP_INITIALIZE_COLOR_TABLE1:       
  PHA
  PHX
  LDA #<(VDP_COLOR_TABLE_BASE1)    ; Byte 1 loading LSB Pattern Table Address
  STA VREG
  LDA #(VDP_WRITE_VRAM_BIT | >VDP_COLOR_TABLE_BASE1) ; Byte 2 loading MSB Pattern Table Address (combined
  STA VREG                                        ; with 0 1 A0 A1 A2 A3 A4 A5
  LDA #<VDP_COLOR                      ; LOAD THE START ADDRESS OF THE COLOR TO ZERO PAGE
  STA VDP_INIT
  LDA #>VDP_COLOR
  STA VDP_INIT_HI
VDP_COLOR_TABLE_LOOP1:
  LDA (VDP_INIT)                  ; LOAD A WITH THE VALUE AT VDP_COLOR_INIT
  STA VDATA                            ; AND STORE IT TO VRAM
  LDA VDP_INIT                    ; LOAD THE LOW BYTE OF VDP_COLOR_INIT ADDRESS INTO A
  CLC                                     ; CLEAR CARRY FLAG
  ...
  STA VDP_INIT_HI                 ; AND STORE THAT BACK INTO THE HIGH BYTE
  CMP #>VDP_END_COLOR                  ; COMPARE IF WE'RE AT THE END OF THE COLOR
  BNE VDP_COLOR_TABLE_LOOP1              ; IF NOT, LOOP AGAIN
  LDA VDP_INIT                    ; COMPARE THE LOW BYTE
  CMP #<VDP_END_COLOR
  BNE VDP_COLOR_TABLE_LOOP1            ; IF NOT EQUAL, LOOP AGAIN
  PLX
  PLA
  RTS

VDP_INITIALIZE_COLOR_TABLE2:       
  PHA
  PHX
  LDA #<(VDP_COLOR_TABLE_BASE2)    ; Byte 1 loading LSB Pattern Table Address
  STA VREG
  LDA #(VDP_WRITE_VRAM_BIT | >VDP_COLOR_TABLE_BASE2) ; Byte 2 loading MSB Pattern Table Address (combined
  STA VREG                                        ; with 0 1 A0 A1 A2 A3 A4 A5
  LDA #<VDP_COLOR                      ; LOAD THE START ADDRESS OF THE COLOR TO ZERO PAGE
  STA VDP_INIT
  LDA #>VDP_COLOR
  STA VDP_INIT_HI
VDP_COLOR_TABLE_LOOP2:
  LDA (VDP_INIT)                  ; LOAD A WITH THE VALUE AT VDP_COLOR_INIT
  STA VDATA                            ; AND STORE IT TO VRAM
  LDA VDP_INIT                    ; LOAD THE LOW BYTE OF VDP_COLOR_INIT ADDRESS INTO A
  CLC                                     ; CLEAR CARRY FLAG
  ...
  STA VDP_INIT_HI                 ; AND STORE THAT BACK INTO THE HIGH BYTE
  CMP #>VDP_END_COLOR                  ; COMPARE IF WE'RE AT THE END OF THE COLOR
  BNE VDP_COLOR_TABLE_LOOP2              ; IF NOT, LOOP AGAIN
  LDA VDP_INIT                    ; COMPARE THE LOW BYTE
  CMP #<VDP_END_COLOR
  BNE VDP_COLOR_TABLE_LOOP2            ; IF NOT EQUAL, LOOP AGAIN
  PLX
  PLA
  RTS
...
VDP_SET_REGISTERS: 
	...
	LDX #$07 ; X = 7..0 
INIT1: 
	LDA ITAB,X ; ITAB[7]..ITAB[0] 
	JSR SREG ; write to R7..R0 
	DEY ; next register down 
	DEX 
	BPL INIT1 ; loop while X >= 0 
	PLY 
	PLA 
	RTS
...
ITAB .BYTE $02,$C2,$0E,$FF       ;INITIALIZE G2 MODE
     .BYTE $03,$76,$03,$0E       ;R0=bit 0 LSB: 0 DISABLE EXT VDP; bit 1: 1 GMODE 2 
                                 ...
                                 ;R3= COLOR TABLE BASE ADDRESS HEX 80 * 40 = 2000_================================== FF
                                 ;R4= PATTERN TABLE BASE ADDRESS HEX 00 * 800 = 0000                                 03
...
VDP_NAME:
   .byte $07,$00,$08,$00,$09,$00,$07,$00   ;ROW 1
   ...
   .byte $00,$00,$00,$00,$00,$00,$00,$00
                                           ;**************************************
   .byte $00,$07,$00,$08,$00,$09,$00,$07   ;COL 5
   .byte $00,$00,$00,$00,$00,$00,$00,$00
   .byte $02,$00,$00,$02,$00,$00,$00,$00
   .byte $00,$00,$00,$00,$00,$00,$00,$00
   ...
   .byte $07,$00,$08,$00,$09,$00,$07,$00    ;COL 20
   .byte $00,$00,$00,$00,$00,$00,$00,$00
   .byte $02,$00,$00,$00,$00,$00,$00,$00
   .byte $00,$00,$00,$00,$00,$00,$00,$00
                                           ;**************************************
